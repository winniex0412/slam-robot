                                   ...
                                   const MotionPlanner& planner)
{
    ...
    */
    // chose a closed frontier
    float tempDis = __FLT_MAX__;
    Point<float> pointDes;
    for(auto ele = frontiers.begin(); ele != frontiers.end(); ele++){
        Point<float> pointAverage = {0, 0};
        for(auto point = ele->cells.begin(); point != ele->cells.end(); point++){
            pointAverage.x += point->x;
            pointAverage.y += point->y;    
        }
        pointAverage.x = pointAverage.x/ele->cells.size();
        pointAverage.y = pointAverage.y/ele->cells.size();
    float dis = std::sqrt((pointAverage.x-robotPose.x)*(pointAverage.x-robotPose.x) + (pointAverage.y-robotPose.y)*(pointAverage.y-robotPose.y));
    if(dis < tempDis){
        tempDis = dis;
        pointDes.x = pointAverage.x;
        pointDes.y = pointAverage.x;
    }
    
    }
    // get a point between pointAverage and robotpose
    //Point<float> goalPoint = {robotPose.x + 0.7*(pointAverage.x-robotPose.x), robotPose.y + 0.7*(pointAverage.y-robotPose.y)}; 
    // check whether this goalPoint is a valid goalPoint
    pose_xyt_t goalPose;
    goalPose.x = robotPose.x + 0.7*(pointDes.x-robotPose.x);
    goalPose.y = robotPose.y + 0.7*(pointDes.y-robotPose.y);
    //planPath
    //planner.isValidGoal(goalPose);
