...
public:
    ...
    */
    ...
    // Debg print function for the grid
    void printGrid();
    void printOccupancyGrid(const OccupancyGrid& map);

    // Calculate a distance between two points (x1, y1) and (x2, y2)
    double calcDist(int x1, int y1, int x2, int y2) const;
    // Finds the nearest obstacle to (x,y) and returns the distance to it in cell coordinates
    double nearestObstacle(const OccupancyGrid& map, int x, int y) const;
    double nearestObstacleBFS(const OccupancyGrid& map, int x, int y) const;
    double nearestObstacleObs(const OccupancyGrid& map, int x, int y, std::vector<std::tuple<int, int>> obs) const;
